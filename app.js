...
function openContractModal(prefill = {}) {
  const selectedId = prefill.customerId || "";
  $("#modalRoot").innerHTML = `<div class="overlay">
    <form class="modal" id="contractForm">
      <div class="modal-head">
        <div><div class="eyebrow">NEW CONTRACT</div><h2>สร้างสัญญา</h2></div>
        <button type="button" class="icon-btn" data-close>×</button>
      </div>

      <div class="form-note">
        <b>เริ่มจากสัญญาได้เลย</b>
        <span>ไม่จำเป็นต้องสร้างลูกค้าก่อน ระบบจะผูกลูกค้าให้อัตโนมัติเมื่อกรอกชื่อ</span>
      </div>

      <label>สินค้า / รายการ
        <input name="product" required placeholder="เช่น iPhone 16 Pro" value="${esc(prefill.product || "")}">
      </label>
      ...
      <div class="customer-row">
        <label>ชื่อลูกค้า
          <input name="customerName" placeholder="ชื่อ-นามสกุล" value="${esc(prefill.customerName || "")}">
        </label>
        <label>เบอร์โทร
      ...
      <div class="customer-row">
        <label>ยอดรวม
          <input name="total" type="number" min="0.01" step="0.01" required placeholder="20000">
        </label>
        <label>รับแล้ว
          <input name="received" type="number" min="0" step="0.01" value="0">
        </label>
      </div>
      ...
      <label>วันครบกำหนดงวดแรก
        <input name="dueDate" type="date" value="${esc(prefill.dueDate || localToday())}">
      </label>
