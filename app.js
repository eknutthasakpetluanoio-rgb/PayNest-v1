...
function contractCard(c){
  ...
  return `<article class="contract-card card">
    ...
    <div class="contract-bottom"><span>${status(c)==="paid"?"✓ ชำระครบแล้ว":"งวดถัดไป "+fmtDate(c.dueDate)}</span>${status(c)==="paid"?`<span class="mini-btn paid-label">ชำระครบ</span>`:`<button class="mini-btn" data-pay="${c.id}">รับชำระ</button>`}</div>
  </article>`;
}
...
function openPayment(id){
  const c=data.contracts.find(x=>x.id===id); if(!c || status(c)==="paid")return;
